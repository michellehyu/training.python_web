...
def sorted_results(key, count, reverse):
    #translate key
    if key == "highscore":
        sort_by = "High Score"
    elif key == "average":
        sort_by = "Average Score"
    elif key == "mostinpection":
        sort_by = "Total Inspections"
    ...
    #take into account of sort is sorted asc by default
    rev = not reverse
    sorted_list = sorted(metadata_list, key=itemgetter(sort_by), reverse=rev)
...
if __name__ == '__main__':
    args = sys.argv
    sort = args[1]
    num_result = int(args[2])
    reverse = True if len(args) == 4 else False
    results = sorted_results(sort, num_result, reverse)
    total_result = {'type': 'FeatureCollection', 'features': []}
    for result in results:
        geojson = get_geojson(result)
        total_result['features'].append(geojson)
    with open('my_map1.json', 'w') as fh:
